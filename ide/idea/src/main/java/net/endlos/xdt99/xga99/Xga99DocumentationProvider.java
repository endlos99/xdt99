...
public class Xga99DocumentationProvider extends AbstractDocumentationProvider {
    ...
    public @Nullable String generateDoc(PsiElement element, @Nullable PsiElement originalElement) {
        if (element instanceof Xga99Labeldef) {  // for some reason, we get Labeldef instead of OpLabel
            final String symbol = ((Xga99Labeldef) element).getName();
            IntWrapper lino = new IntWrapper(0);
            final String def = Xga99Util.getLabeldefText((Xga99Labeldef) element, lino);  // modified lino
            if (def == null)
                return null;
