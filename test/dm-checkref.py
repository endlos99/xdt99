...
from utils import xdm, r, clear_env, delfile, check_files_eq, check_file_matches, check_bin_text_eq
...
def runtest():
    ...
    for fn in ['V1', 'V16', 'V126', 'V127', 'V128', 'V254', 'V10R',
        ...
        xdm(Disks.work, '-t', '-e', fn, '-o', Files.output)
        check_files_eq('TIFiles', Files.output, r(fn + '.tfi'), 'PROGRAM', Masks.tifiles_date)
    ...
    for fn in ['V16', 'V127', 'V64V', 'V10R', 'F129', 'F64V', 'F10R']:
        ...
        check_files_eq('v9t9', Files.output, r(fn + '.v9t9'), 'PROGRAM',
    ...
    for fn in ['F1', 'F6']:
        ...
        check_files_eq('Frag Disk', Files.output,
                       r('FRAG' + fn), 'DIS/VAR127')
    xdm(Disks.work, '-d', 'F1')
    xdm(Disks.work, '-e', 'F6', '-o', Files.output)
    check_files_eq('Frag Disk', Files.output, r('FRAGF6'), 'DIS/VAR127')
    shutil.copyfile(Disks.frag, Disks.work)
    xdm(Disks.work, '-a', Files.output, '-n', 'COPY', '-f', 'DIS/VAR127')
    xdm(Disks.work, '-e', 'F1', '-o', Files.output)
    check_files_eq('Frag Disk', Files.output, r('FRAGF1'), 'DIS/VAR127')
